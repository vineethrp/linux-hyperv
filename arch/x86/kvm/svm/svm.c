...
static int hv_enable_direct_tlbflush(struct kvm_vcpu *vcpu)
{
	 ...
	 * vcpus in the VM shares same assist page.
	 */
	if (!*p_hv_pa_pg)
...
#if IS_ENABLED(CONFIG_HYPERV)
	if (svm->vmcb->hv_enlightenments.hv_vp_id != vcpu->arch.hyperv.vp_index) {
		svm->vmcb->hv_enlightenments.hv_vp_id = vcpu->arch.hyperv.vp_index;
			     ...
			     int root_level)
{
	...
	if (npt_enabled) {
		...
		if (kvm_x86_ops.tlb_remote_flush)
			kvm_update_arch_ept_pointer(vcpu->kvm, vcpu, cr3);
